...
17. PROIBIÇÃO ABSOLUTA DE CIFRÕES INTERNOS ($ DENTRO DE $$): Dentro de um bloco de display math `$$ ... $$`, é TERMINANTEMENTE PROIBIDO colocar novos cifrões `$`. Tudo o que está entre `$$` e `$$` já está em modo matemático. Escreva `$$ P(A) \\geq 0 $$` e NUNCA `$$ $P(A)$ \\geq 0 $$` ou `$$ \\textbf{Axioma 1:}$ P(A) \\ge 0 $$`.
18. FECHAMENTO CORRETO DE COMANDOS (NUNCA ESCAPAR CHAVES DE AGRUPAMENTO): Chaves de comandos como `\\text{...}` ou `\\frac{...}{...}` DEVEM ser fechadas com chaves simples `}`. É TERMINANTEMENTE PROIBIDO usar `\\}` para fechar argumentos de comandos (ex: NUNCA escreva `\\frac{\\text{Texto}\\}}{\\text{Total}\\}`, escreva SEMPRE `\\frac{\\text{Texto}}{\\text{Total}}`).
"""
...
6. SIMULADORES E GRÁFICOS INTERATIVOS (MAPEAMENTO GENEROSO): Sempre que o subtópico tratar de representação gráfica (como barras, colunas, pizza/setores, linhas, séries temporais, histogramas, boxplots, dispersão, regressão, densidades de probabilidade) ou intuição geométrica, preencha OBRIGATORIAMENTE o campo `simuladores_interativos_recomendados` com propostas detalhadas de simulação com Plotly. Especifique variáveis estatísticas reais (ex: frequências de categorias, bins, quartis, médias) e controles práticos para o aluno explorar visualmente.
...
3. Mapeamento Generoso e Preciso de Simuladores Interativos (MANDATÓRIO):
   - Analise CADA página/subtópico da aula.
   - Para CADA subtópico que envolva representação visual, gráficos estatísticos (barras, colunas, setores/pizza, histogramas, polígonos de frequências, ogivas, boxplots, dispersão, correlação, regressão, séries temporais, curvas de densidade ou distribuições), MAPEIE OBRIGATORIAMENTE um simulador interativo no campo 'simuladores_da_aula'.
   - Se a aula for sobre 'Gráficos', 'Visualização de Dados' ou 'Estatística Descritiva', MAPEIE UM SIMULADOR PARA CADA SUBTÓPICO PERTINENTE (gerando de 2 a 5 simuladores por aula conforme a necessidade visual). É PROIBIDO economizar simuladores quando o tema for gráfico ou visual!
