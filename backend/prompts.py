...
6. SIMULADORES E GRÁFICOS INTERATIVOS: Avalie se o subtópico demanda visualização gráfica interativa (ex: gráficos de colunas/barras, histogramas, distribuições de probabilidade, gráficos de dispersão com reta, diagnóstico de resíduos, boxplots, diagramas de setores, convergência no TCL). Se o tema for visual ou tratar de gráficos e distribuições, preencha OBRIGATORIAMENTE o campo `simuladores_interativos_recomendados` com propostas ricas e detalhadas de laboratórios interativos com controles dinâmicos. Em aulas sobre gráficos e visualização de dados, TODOS os subtópicos sobre tipos de gráficos devem ter simulador recomendado. Se o subtópico for puramente histórico ou qualitativo sem dados ou gráficos, retorne estritamente `null`.
...
3. Mapeamento Generoso e Rico de Simuladores e Gráficos Interativos (OBRIGATÓRIO): Mapeie um simulador interativo no campo 'simuladores_da_aula' para CADA subtópico que apresente conceitos visuais, geométricos, tipos de gráficos ou distribuições. Em aulas sobre construção e interpretação de gráficos, estatística descritiva, distribuições ou probabilidade, mapeie de 2 a 5 simuladores interativos (um para cada tipo de gráfico abordado nos subtópicos: colunas/barras, histograma, boxplot, dispersão, setores, etc.). É PROIBIDO limitar a aula a apenas 1 ou 2 simuladores quando múltiplos gráficos forem ensinados. Se a aula for puramente teórica e não contiver gráficos, mapeie pelo menos 1 simulador.
